...
addr_ranges = [ [ '00000000', '0007FFFF' , 8192 , 0 , 4 , 0 ] ,   # Part 1 of 16MB repeated  8 times, and covers till 7FFF FFFF #  Split into 3 parts to exclude Data and Logic part (part 2). That part is extrated slow and via markers
                ...
                [ '08000000', '08000FFF' , 1024 , 0 , 0 , 0 ] ,  # 4KB - 0 [8 or 9] X  x x x 0   X A B C
                [ '08010080', '0801037F' , 1024 , 0 , 0 , 0 ] ,  # 2nd 4KB first piece ; gaps not readable - 0 [8 or 9] X  x x x 1   X D E F
                [ '08010400', '0801047F' , 1024 , 0 , 0 , 0 ] ,  # 4KB in pieces ; gaps not readable - 0 [8 or 9] X  x x x 1   X D E F
                [ '08010580', '080108FF' , 1024 , 0 , 0 , 0 ] ,  # 4KB in pieces ; gaps not readable - 0 [8 or 9] X  x x x 1   X D E F
                [ '08010980', '08010AFF' , 1024 , 0 , 0 , 0 ] ,  # 4KB in pieces ; gaps not readable - 0 [8 or 9] X  x x x 1   X D E F
                [ '08010C80', '08010D7F' , 1024 , 0 , 0 , 0 ] ,  # 4KB in pieces ; gaps not readable - 0 [8 or 9] X  x x x 1   X D E F
                [ '08010E00', '08010EFF' , 1024 , 0 , 0 , 0 ] ,  # 4KB in pieces ; gaps not readable - 0 [8 or 9] X  x x x 1   X D E F
              ...
              #'! '0C080000 , '0C081F0F' immediately halts  the processor [0C and 0D has same data
                [ '0C081F10', '0C08283F' , 256  , 0 , 0 , 0 ] ,  #
                [ '0E000000', '0E000FFF' , 256  , 0 , 0 , 0 ],    # Fast counters with dominently zeros; not sure of the boundary (seems few KB)- same pattern till 0FFF FFFF (most likely due to dont care) 
                ...
                [ '1C000000', '1C000FFF' , 256 , 0 , 0 , 0 ],    # Similar to 0E000000; This pattern contiues till 1FFFFFFF
                [ '20000000', '20000FFF' , 256 , 0 , 0 , 0 ],    # Fast moving data - 10% non-zero but moving - some patterns repeated more frequently; eg 0c07b35c, 010c0db0, 1f000db4
                [ '30000000', '30000FFF' , 256 , 0 , 0 , 0 ],    # Fast moving - same as 20000000 except the default pattern 0000FFFF insted of 0000 - continues till 37FFFFFF
                [ '38000000', '38000FFF' , 256 , 0 , 0 , 0 ],    # Fast moving - but only changes 6 bytes of dword - '6E' remains constant for entire region - continues till 3FFFFFFF
                [ '40000000', '4000007F' , 64 , 0 , 0 , 0 ],     # Only 128 bytes - Top 5 bits '0100 0' middle 20 dont care, last 7 - covers till 47FF FFFF
                [ '48000000', '48000FFF' , 256 , 0 , 0 , 0 ] ,     # Fast moving sparse data - boundary not sure - trend repeated till 4BFFFFFF
                [ '4C000000', '4C000FFF' , 256 , 0 , 0 , 0 ],    # Fast moving data - default pattern 0000FFFF - continues till 4DFFFFFF
                [ '4E000000', '4E000FFF' , 256 , 0 , 0 , 0 ],    # Similar to 48000000 to 4BFFFFFF - covers till 4FFFFFFF
               #! '50000000 -  57FFFFFF is same as 00000000 to 07FFFFFF
                ...
                [ '60000000', '60001FFF' , 256 , 0 , 0 , 0],    # 8KB space filled with the own address of each dword (data changeable via debugger)
                [ '60002000', '600027FF' , 256 , 0 , 0 , 0],
