...
##            x[4][-1].append(':')
            ...
            instr_size= int(x[3][i][3],16)+7  # dwords count
            print "instr_size = " + str(instr_size)
            instr_code = x[3][i][4:]
            print "instr_code =" +instr_code
            ...
            if instr_code == '8012': #...................>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> ONS
                if x[3][i+4][:2] != '8e':
                    ...
                    bit_number = int(x[3][i+4][6:],16)
                    if x[3][i+6][:2] == '8f': # it means a physical tag is being used 
                        potential_tag = '0c00'+ str(hex((int(x[3][i+5][4:],16)*4)))[2:].zfill(4)
                    else:
                        potential_tag = str(hex(int(x[3][i+5][2:],16)*4))[2:].zfill(8)
                    bit_number = int(x[3][i+4][4:],16)
                    print "potential tag = " + potential_tag
                    for y in coreDT:
                        if potential_tag == y[2]:
                            tag_string = copy.copy(y[1]) 
                            inst_and_tag = instr_string + "--"+tag_string + "."+ str(bit_number)
                ...
                if x[3][i+4][:2] != '8e':
                    ...
                    for y in coreDT:
                        if potential_tag == y[2]:
                            tag_string = copy.copy(y[1])
                            inst_and_tag = instr_string + "--"+"outputBit="+tag_string + "."+ str(bit_number)
##                            x[4][-1].append(inst_and_tag)
                            break
                    # now finding storageBit
                    bit_number = int(x[3][i+4+first_tag_size][6:],16)
                    if x[3][i+6+first_tag_size][:2] == '8f': # it means a physical tag is being used for storageBit
                        potential_tag = '0c00'+ str(hex(int(x[3][i+5+first_tag_size][4:],16)*4)[2:]).zfill(4)
                        
                    else:
                        potential_tag = str(hex(int(x[3][i+5+first_tag_size][3:],16)*4)[2:]).zfill(8)
                        
                    print "potential tag = " + potential_tag
                    for y in coreDT:
                        if potential_tag == y[2]:
                            tag_string = copy.copy(y[1])
                            inst_and_tag = inst_and_tag + "--"+"storageBit="+tag_string + "."+ str(bit_number)
                            x[4][-1].append(inst_and_tag)
                            break
                    i+=instr_size
                    continue
