...
print (timestr)
print "Size = " + str(end_decimal - start_decimal +1)+" bytes" +   "   Estimated time =" + str((end_decimal - start_decimal +1)*sleep_duration/chunk_size) + " secs"
testfile = open(dest_folder+"code_mem"+ " _time_"+str(timestr) + "_mem_from_"+hex(starting_address)+"_to_"+hex(ending_address)+".bin","ab")
x= start_decimal
while(x < end_decimal):
